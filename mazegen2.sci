...
//      v               Initial vertex.
//      random_choice   Randomly choose an edge if multiple minimal edges available.
//      verbose         
//      write_to_file   
//      filename
...
function [r] = prim(A, v, random_choice, verbose, write_to_file, filename)
    ...
    not_connected = max(A) + 1;
    A(find(A == 0)) = not_connected;
    weight = A(v, :);
    visited(v) = 1;
    while (~isempty(find(visited == 0)))
        vertices_not_visited = find(visited == 0);
        adjs_with_min_weight = ...
            intersect( find(weight == min(weight(vertices_not_visited))), ...
        ...
        indices = grand(1, 'prm', 1:length(adjs_with_min_weight));
